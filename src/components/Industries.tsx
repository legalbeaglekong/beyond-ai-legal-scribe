...
import industryEmployment from "@/assets/industry-employment.jpg";
...
const Industries = () => {
  ...
  const industries = [
    ...
    { title: t("industries.transportation"), description: t("industries.transportationDesc"), highlights: ["Transportation Infrastructure", "Logistics & Supply Chain", "Cross-border Transportation"], route: "/industry/transportation", image: industryTransportation.url },
    ...
    { title: t("industries.employment"), description: t("industries.employmentDesc"), highlights: ["MOM Compliance", "AI Use Policy in HR decision making", "Executive Separations & Investigations"], route: "/singapore-employment-law", image: industryEmployment },
            ...
            {industries.map((industry, index) => (
              <div key={index} className="group flex flex-col">
                ...
                <div className="mt-auto">
                  {industry.link ? (
                    <a href={industry.link} target="_blank" rel="noopener noreferrer" className="text-sm text-foreground link-underline inline-flex items-center" aria-label={`Explore ${industry.title} services`}>
                      {`Explore ${industry.title}`} <ArrowRight className="ml-1 h-3 w-3" />
                    </a>
                  ) : industry.route ? (
                    <Link to={industry.route} className="text-sm text-foreground link-underline inline-flex items-center" aria-label={`Explore ${industry.title} services`}>
                      {`Explore ${industry.title}`} <ArrowRight className="ml-1 h-3 w-3" />
                    </Link>
                  ) : null}
                </div>
