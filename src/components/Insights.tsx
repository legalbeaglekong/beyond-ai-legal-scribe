...
const Insights = () => {
  ...
  const latestPosts = [
    { title: "Supply Chain Resilience, Advance Air Mobility and the Singapore Lighthouse with Emerson Xu", excerpt: "In-depth discussion on supply chain innovation, advanced air mobility developments, and Singapore's role as a technological lighthouse.", date: "December 15, 2024", readTime: "45 min listen", category: "Podcast", url: "#", isVideo: true, embedUrl: "https://open.spotify.com/embed/episode/7kQWg81TEuuZHV5OmDk172/video?utm_source=generator" },
    { title: "AI Clauses and IP Protection: What You Don't Know Can Hurt You with Junus Eu", excerpt: "Essential insights into AI-related contractual clauses and intellectual property protection strategies.", date: "December 10, 2024", readTime: "40 min listen", category: "Podcast", url: "#", isVideo: true, embedUrl: "https://open.spotify.com/embed/episode/7C5YHD3BrMXEe9fOT0bv1V?utm_source=generator" },
    { title: "Your AI Just Threatened You. Now What?", excerpt: "When AI Refuses to Die: Legal Reflections on Blackmail, Shutdown Resistance & the Future of AI Governance", date: "May 29, 2024", readTime: "8 min read", category: "AI Law", url: "https://beyondhorizons.substack.com/p/your-ai-just-threatened-you-now-what" },
    ...
    <section id="insights" className="section-padding bg-background">
      <div className="max-w-6xl mx-auto container-padding">
        <div className="text-center mb-16">
          ...
          <p className="text-sm uppercase tracking-[0.3em] text-muted-foreground mb-4 font-sans">{t("insights.sectionLabel")}</p>
          <h2 className="text-3xl md:text-5xl font-serif font-bold text-foreground mb-6">{t("insights.sectionTitle")}</h2>
          <p className="text-muted-foreground max-w-2xl mx-auto leading-relaxed mb-8">{t("insights.sectionDesc")}</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button variant="default" size="default" onClick={() => navigate("/market-insights")}>
              {t("insights.readInsights")} <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
            <Button variant="outline" size="default" className="border-accent text-accent hover:bg-accent hover:text-accent-foreground" asChild>
              <a href="https://beyondhorizons.substack.com/" target="_blank" rel="noopener noreferrer">
                {t("insights.visitSubstack")} <ExternalLink className="ml-2 h-4 w-4" />
              </a>
            </Button>
          </div>
        </div>
            ...
            <Card key={index} className="border-border/30 bg-card card-lift group">
              <CardContent className="p-6">
                ...
                {post.isVideo && post.embedUrl && (
                  <iframe title={post.title} style={{ borderRadius: '8px' }} src={post.embedUrl} width="100%" height="200" frameBorder="0" allowFullScreen allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy" />
                )}
        ...
        <div className="border border-border/30 rounded p-10 text-center">
          <h3 className="text-xl font-serif font-bold text-foreground mb-4">{t("insights.stayUpdated")}</h3>
          <p className="text-sm text-muted-foreground mb-6 max-w-xl mx-auto">{t("insights.stayUpdatedDesc")}</p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Button variant="default" size="default" asChild>
              <a href="https://beyondhorizons.substack.com/s/beyond-legal" target="_blank" rel="noopener noreferrer">
                {t("insights.subscribeLegal")} <ExternalLink className="ml-2 h-4 w-4" />
              </a>
            </Button>
            <Button variant="outline" size="default" className="border-accent text-accent hover:bg-accent hover:text-accent-foreground" asChild>
              <a href="https://beyondhorizons.substack.com/" target="_blank" rel="noopener noreferrer">
                {t("insights.businessTech")} <ExternalLink className="ml-2 h-4 w-4" />
              </a>
            </Button>
          </div>
        </div>
